...
distanceToPatch <- function(object,
  ...
  cur_out <- bplapply(
    ...
    function(x){
      ...
      cur_obj[[name]] <- NA
      if (sum(!is.na(colData(cur_obj)[[var]])) == 0) {
        return(cur_obj)
      }

      # calculate the distances for all against all cells
      distance <- dist(as.matrix(colData(cur_obj)[,coords]))
      distance <- as.matrix(distance)

      # select only those columns (cells) that are part of the patch
      pos_dist <- distance[,!is.na(colData(cur_obj)[[var]])]
      ...
      neg_dist <- distance[,is.na(colData(cur_obj)[[var]])]
