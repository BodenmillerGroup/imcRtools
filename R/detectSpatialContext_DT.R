...
detectSpatialContext_DT <- function(object,
  ...
  cur_dat_dt <- as.data.table(cur_dat)
  
  cur_dat_dt[,row := .I]
  
  cur_dat_melt <- melt(cur_dat_dt, id.vars = "row")
  
  cur_dat_melt <- cur_dat_melt[order(row,-value)]
  
  cur_dat_melt <- cur_dat_melt[,cumsum := cumsum(value),by=row]
  
  out_dat_dt <- cur_dat_melt[,.(
    spatial_context = if (sum(cumsum) > 0) paste(sort(as.numeric(str_replace(variable[1:(sum(cumsum < threshold)+1)],"X",""))),collapse= "_") else "NA"
  ), by = row]
  
  out_dat <- out_dat_dt$spatial_context
  out_dat[out_dat == "NA"] <- NA
