...
k-nearest neighbor (KNN) graph, which we can retrieve using the
`buildSpatialGraph` and `aggregateNeighbors` functions. It is recommended to use
a higher k in the KNN graph for SC than for CN detection [@Bhate2022].

Subsequently, the CN fractions are sorted from high-to-low and the SC of each
cell is assigned as the set of CNs that cumulatively exceed a user-defined
fraction threshold.
