...
The output additionally has the cond_ratio column. The cond_ratio is the ratio of cells of type A with at least one neighbor of type B. 
We can use the `plotInteractionsCozi()` function to create a dotplot where the color of the dots is the z-score and the size of the dots is the 
cond_ratio. 
...
contributed the zscore and cond_ratio calculation within the `testInteractions()` function, and
implemented the `plotInteractionsCozi` function.
