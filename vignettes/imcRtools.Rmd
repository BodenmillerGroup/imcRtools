...
## Distance statistics to cells of interest
...
2.  `method = "conditional"`: The count is divided by the number of cells
    ...
    This method additionall outputs the conditional cell ratio 
    \code{cond_ratio}, which is the fraction of cells of type A that 
    have at least one neighbor of type B.
...
```{r plotInteractions-filter data}
library(dplyr)
...
```{r plotInteractions-edge aesthetics}
library(ggraph)
...
```{r plotInteractions-node aesthetics}
plotInteractions(out,
                 pancreasSCE,
                 group_by = "ImageNb",
                 label = "CellType",
...
For more information see \link[ggraph]{ggraph}.

```{r plotInteractions-layout}
# Default is "circle"
plotInteractions(out,
                 pancreasSCE,
                 group_by = "ImageNb",
...
```{r plotInteractions-return data}
# return data
plotInteractions(out, pancreasSCE, "CellType", "ImageNb",
                return_data = TRUE)
```

### Visualizing interactions when choosing method = "conditional"

The `plotInteractionsCozi()` function can be used to visualize interactions after running `testInteractions()` with the "conditional" method.

We first run the count and tesInteractions() functions again with the "conditional" method. 
...
The output additionally has the cond_ratio column. The cond_ratio is the ratio of cells of type A with at least one neighbor of type B in the image. 
When performing conditional normalization, it can be helpful to visualize both the z-score and the cond_ratio simultaneously in a dotplot to help 
interpreting results. With that, you know how many cells of type A neighbor a cell of type B (cond_ratio), and how strong this interaction is (z-score). 
For example, if the cond_ratio is very low (small dot) but the z-score is very high, this means that a small amount of cells of type A are strongly 
surrounded by cells of type B. 
We show how to plot the results in the following:
```{r plotInteractionsCozi}
library(ggplot2)
...
plot_data <- as.data.frame(out) %>%
  select(group_by, from_label, to_label, sig, zscore, cond_ratio) %>%
  { if (filter_sig) filter(., sig) else . } %>%
  na.omit()

p <- ggplot(plot_data, aes(from_label, to_label)) +
  geom_point(aes(size = cond_ratio, color = zscore), shape = 16) +
  scale_size_continuous(range = c(1, 10)) +
  scale_color_gradient2(
    low  = brewer.pal(11, "RdBu")[10],
    mid  = "white",
  ...
  labs(
    ...
    x = "Cell Type (From)", y = "Cell Type (To)",
    size = "Conditional Ratio", color = "Z-score"
  ) +
  theme(
...
contributed the zscore and cond_ratio calculation within the `testInteractions()` function.
