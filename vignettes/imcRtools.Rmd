...
4. `method = "interaction"`: This method normalizes the interaction count 
    between two cell types by the number of interactions originating from 
    cells of type A. The final interaction score can be interpreted as a 
    normalized measure of how frequently cells of type A interact with 
    cells of type B.
...
(based on sigval) exceeds 25% and tag the edges as attraction or repulsion.
...
summary_df <- out %>%
  ...
  mutate(
    ...
    int_type = case_when(
      fraction_sig > 0.25 ~ "attraction",
      fraction_sig < -0.25 ~ "repulsion",
      TRUE ~ NA_character_
    )
  ) %>%
...
or `testInteractions`, averaged per cell-pair. In our example, the "ct" value
refers to the average neighboring cells of cell type B from cell type A.
