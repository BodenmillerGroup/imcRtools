...
The cell assignments derived from clustering cells based on their
environment can be interpreted as **cellular neighborhoods (CNs)** (
\href{https://doi.org/10.1016/j.cell.2020.07.005}{ Schürch C. et al.,
Coordinated Cellular Neighborhoods Orchestrate Antitumoral Immunity at the
Colorectal Cancer Invasive Front, Cell, 2020}).
...
The term SC was coined by
\href{https://doi.org/10.1016/j.cels.2021.09.012}{
Salil S. et al., Tissue schematics map the specialization of immune tissue
motifs and their appropriation by tumors, Cell Systems, 2022} and
describes tissue regions in which distinct CNs may be interacting.

The `detectSpatialContext` function relies on CN fractions for each cell
in a k-nearest neighbor graph, which we can retrieve using the
`buildSpatialGraph` and `aggregateNeighbors` functions. Subsequently,
the CN fractions are sorted from high-to-low and the SC of each cell is
assigned as the set of CNs that cumulatively exceed a user-defined
fraction threshold.

```{r detectSpatialContext}
# Generate additional k-nearest neighbor graph with higher k
pancreasSCE <- buildSpatialGraph(pancreasSCE, img_id = "ImageNb",
                                 type = "knn",
...
also returns a `data.frame` entry to `metadata(object)` containing group
...
akin to *CN combination maps* in
href{<https://doi.org/10.1016/j.cels.2021.09.012>}{ Salil S. et al.,
Tissue schematics map the specialization of immune tissue motifs and
their appropriation by tumors, Cell Systems, 2022}, based on symbolic
...
`aggregateNeighbors` function and contributed to developing the spatial
clustering approach.
